...
// Function to preload brand images
function preloadBrandImages() {
const defaultImg = new Image();
defaultImg.onload = () => {
imageCache.set(‘default’, defaultImg);
...
brandIds.forEach(brandId => {
  const img = new Image();
  img.onload = () => {
    imageCache.set(brandId.toString(), img);
    console.log(`Loaded brand image: ID ${brandId}`);
    updateVisibleStations();
  };
  img.onerror = () => {
    console.warn(`Failed to load brand image: images/${brandId}.png`);
  };
  img.src = `images/${brandId}.png`;
});
```
...
if (!listUl) return;
let sortHtml = `<div style="display:flex;justify-content:flex-end;align-items:center;margin-bottom:10px;gap:12px;"> <label><input type="radio" name="sort-mode" value="price" ${sortMode==="price"?"checked":""}> Sort by Price</label> <label><input type="radio" name="sort-mode" value="distance" ${sortMode==="distance"?"checked":""}> Sort by Distance</label> </div>`;
...
listUl.innerHTML = stations.map(site => `
  <li class="station-list-item" data-siteid="${site.siteId}">
    <div style="display:flex;align-items:center;">
      <img src="images/${site.brand || 'default'}.png"
        alt="${site.name}" 
        style="height:48px;width:48px;border-radius:50%;background:#fff;object-fit:contain;margin-right:14px;box-shadow:0 1px 2px rgba(0,0,0,0.07);"
      ...
      <div style="margin-left:auto;font-weight:700;font-size:1.18em;color:${site.isCheapest ? '#00C851' : '#1b9b57'};">
...
allSites.forEach(site => {
  ...
  if (
    ...
    // Check if this is the cheapest station in viewport
    const isCheapest = cheapestStation && site.S === cheapestStation.S;

    // Get the cached image or use default
    let glyphImage = null;
    if (s.brand && imageCache.has(s.brand.toString())) {
      glyphImage = imageCache.get(s.brand.toString());
    } else if (imageCache.has('default')) {
      glyphImage = imageCache.get('default');
    }
...
content.innerHTML = `<div class="feature-card-title">${station.name}</div> <div class="feature-card-address" style="cursor:pointer;color:#387CC2;text-decoration:underline;" onclick="window.open('https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(station.address + ', ' + (station.suburb || ''))}', '_blank')"> ${station.address}${station.suburb ? ', ' + station.suburb : ''} </div> <div class="feature-card-distance">${station.price.toFixed(1)} (${FUEL_TYPES.find(f=>f.key===currentFuel).label})</div>`;
}
...
`<li class="suburb-list-item" style="padding:12px;cursor:pointer;border-bottom:1px solid #eee;"  data-lat="${site.Lat}" data-lng="${site.Lng}" data-name="${site.P}"> <span style="font-weight:500">${site.P}</span>  <span style="color:#888;">${site.Postcode || ''}</span> </li>`
).join(’’);
...
// — Map startup —
preloadBrandImages(); // Load default image first
